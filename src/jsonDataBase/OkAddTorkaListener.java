 ...
 */
public class OkAddTorkaListener implements ActionListener {
	private Table t;
	/**
	 * Constructor that sets table that is going to get a tuple
	 * @param t - table that gets new tuple
	 */
	public OkAddTorkaListener(Table t) {
		this.t=t;
	}
	...
	/**
	 * Go through all Components that contains data, checks if they are valid
	 * and in the end makes a tuple and adds it to relational dataBase or redirect it to actionPerformedOnRelational-method
	 */
	private void actionPerformedOnRelational() {
		...
		for(Atribut a : t.getPolja()){	
			...
			case BOOLEAN:
				if(!a.getisNull()){
					if(((JRadioButton)podaci.get(a.getName().getCode()+"N")).isSelected()){
						JOptionPane.showMessageDialog(atf, ((JRadioButton)podaci.get(i+2)).getText()+Sistem.getInstance().getTranslate("Value_Can_Not_Be_Null"), "Error", JOptionPane.ERROR_MESSAGE);	
						valid = false;
					}
				}
				v.setCode(a.getName().getCode());
